...
import json
import time
from common import connect_to_box, error, settings, debug, status_update, ssh_cmd
from common import load_json, get_port_ovs_id_tag, execute_cmd, dump_json
...
def capture_network_packets (params, hop_list):
    ...
    for hop in net_info['hops']:
        ...
        cmd += 'tcpdump -v icmp -i %s -c %d -l > %s 2>&1' % (dev, params['count'], filename)
        pid = subprocess.Popen(cmd, shell=True).pid
        net_info['pids'].append(pid)
        status_update('net: tcpdump launched with pid %d for interface %s' % (pid, dev))
    pass
...
def capture_packets (params, tag='src', src_tag=None):
    ...
    for port in port_info['ports'].keys():
        ...
        if os.path.isfile(filename):
            os.remove(filename)
        cmd = 'sudo tcpdump -v icmp -i %s -c %d -l > %s 2>&1' % (intf, params['count'], filename)
        pid = subprocess.Popen(cmd, shell=True).pid
        port_info['pids'].append(pid)
        status_update('%s: tcpdump launched with pid %d for interface %s' % (tag, pid, intf))
...
def check_args(params):
    ...
    parser.add_argument('--json_file', dest='json_file', help='JSON file having info of installation (required)', type=str, required=True)
    parser.add_argument('--count', dest='count', help='ping count', type=int, default=COUNT)
    parser.add_argument('--timeout', dest='timeout', help='ping timeout (-W option of ping) in seconds', type=int, default=2)
    parser.add_argument('--router', dest='router', help='router to be used for the test', type=str, required=True)
    parser.add_argument('--path_file', dest='path_file', help='Test results are printed in this file in JSON format', type=str, default='path.json')
...
def path (params):
    ...
    if src_info['tag'] == dst_info['tag']:
        ...
        if len(next_hop_list) == 0:
            error('Could not find next hop list from %s to %s' % (src_ip, dst_ip))
